...
class ImageUpload extends AliBase {
    ...
    public function setImageType(string $imageType){
        ...
        if(($length > 0) && ($length <= 8)){
            ...
            throw new AliPayException('图片格式不合法', ErrorCode::ALIPAY_PARAM_ERROR);
        }
    }
    ...
    public function setImageName(string $imageName){
        ...
        if(($length > 0) && ($length <= 128)){
            ...
            throw new AliPayException('图片名称不合法', ErrorCode::ALIPAY_PARAM_ERROR);
        }
    }
    ...
    public function setImageContent(string $imageContent){
        if(strlen($imageContent) > 0){
            ...
            throw new AliPayException('图片二进制内容不合法', ErrorCode::ALIPAY_PARAM_ERROR);
        }
    }
    ...
    public function setImagePid(string $imagePid){
        if(ctype_digit($imagePid) && (strlen($imagePid) <= 16)){
            ...
            throw new AliPayException('图片所属的partnerId不合法', ErrorCode::ALIPAY_PARAM_ERROR);
        }
    }

    public function getDetail() : array {
        if(!isset($this->biz_content['image_type'])){
            throw new AliPayException('图片格式不能为空', ErrorCode::ALIPAY_PARAM_ERROR);
        }
        if(!isset($this->biz_content['image_name'])){
            throw new AliPayException('图片名称不能为空', ErrorCode::ALIPAY_PARAM_ERROR);
        }
        if(!isset($this->biz_content['image_content'])){
            throw new AliPayException('图片二进制内容不能为空', ErrorCode::ALIPAY_PARAM_ERROR);
        }
