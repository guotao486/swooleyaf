...
class PayPage extends AliBase {
    ...
    public function setTimeExpire(int $timeExpire){
        if($timeExpire > time()){
            ...
            throw new AliPayException('绝对超时时间不合法', ErrorCode::ALIPAY_PARAM_ERROR);
        }
    }
    ...
    public function setGoodsType(string $goodsType){
        if(in_array($goodsType, ['0', '1'])){
            ...
            throw new AliPayException('商品主类型不合法', ErrorCode::ALIPAY_PARAM_ERROR);
        }
    }
    ...
    public function setQrPayMode(string $qrPayMode){
        if(in_array($qrPayMode, ['0', '1', '2', '3', '4'])){
            ...
            throw new AliPayException('扫码支付方式不合法', ErrorCode::ALIPAY_PARAM_ERROR);
        }
    }
    ...
    public function setIntegrationType(string $integrationType){
        if(in_array($integrationType, ['ALIAPP', 'PCWEB'])){
            ...
            throw new AliPayException('页面集成方式不合法', ErrorCode::ALIPAY_PARAM_ERROR);
        }
    }
