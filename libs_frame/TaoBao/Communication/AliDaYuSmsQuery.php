...
class AliDaYuSmsQuery extends TaoBaoBase {
    ...
    public function setQueryDate(string $queryDate){
        if(strlen($queryDate) == 8){
            ...
            throw new SmsException('发送日期不合法', ErrorCode::SMS_PARAM_ERROR);
        }
    }
    ...
    public function setPage(int $page){
        if($page >= 1){
            ...
            throw new SmsException('页码必须大于0', ErrorCode::SMS_PARAM_ERROR);
        }
    }
    ...
    public function setLimit(int $limit){
        if(($limit >= 1) && ($limit <= 50)){
            ...
            throw new SmsException('每页数量必须在1-50之间', ErrorCode::SMS_PARAM_ERROR);
        }
    }

    public function getDetail() : array {
        if(!isset($this->reqData['rec_num'])){
            throw new SmsException('接收号码必须填写', ErrorCode::SMS_PARAM_ERROR);
        }
        if(!isset($this->reqData['query_date'])){
            throw new SmsException('发送日期必须填写', ErrorCode::SMS_PARAM_ERROR);
        }
